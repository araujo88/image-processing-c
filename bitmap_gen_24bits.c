...
void set_DIB_header(unsigned char *buffer, size_t *i)
{
    ...
    write_bytes(buffer, i, 1, 1, 0x18);       // number of bits per pixel (24 bits)
...
int main(void)
{
    ...
    write_bytes(buffer, &i, 0, 3, 0x0, 0x0, 0xFF);   // Red, Pixel (x=0, y=1)
    write_bytes(buffer, &i, 0, 3, 0xFF, 0xFF, 0xFF); // White, Pixel (x=1, y=1)
    write_bytes(buffer, &i, 0, 2, 0x0, 0x0);         // Padding for 4 byte alignment (could be a value other than zero)
    write_bytes(buffer, &i, 0, 3, 0xFF, 0x0, 0x0);   // Blue, Pixel (x=0, y=0)
    write_bytes(buffer, &i, 0, 3, 0x0, 0xFF, 0x0);   // Green, Pixel (x=1, y=0)
    write_bytes(buffer, &i, 0, 2, 0x0, 0x0);         // Padding for 4 byte alignment (could be a value other than zero)
